...
`riot-account-rank` additionally needs `LOL_SPLIT_START_TIMESTAMP` (Unix seconds) to count placement matches already played this split for the MD5 flow — Riot's League-V4 has no direct "games until placement" field, so this is derived via Match-V5 match-id counts since the split start. **Must be updated manually every time Riot starts a new split** (roughly twice a year); until it's set, `matches_remaining` defaults to 5 (i.e. "assume no placements played yet").
