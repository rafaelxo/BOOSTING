-- Fix de migration 116: orders tem a check constraint orders_price_sum
-- (total_price = round(base_price + extras_price - discount_price, 2)).
-- apply_order_drop só atualizava total_price ao reduzir pela metade,
-- deixando base_price/extras_price/discount_price com os valores
-- originais -- violava a constraint (100 base + 0 extras - 0 desconto
-- != 50 total). Ao reduzir o preço (>=50% concluído), a composição
-- original (base/extras/desconto) não é recalculada proporcionalmente --
-- simplesmente absorvida inteira em base_price, com extras/desconto
-- zerados, pra satisfazer a constraint sem risco de mismatch de
-- arredondamento entre as três colunas escaladas independentemente.
-- Quando o preço NÃO muda (<50% concluído), base/extras/desconto também
-- não mudam -- só o branch >=50% precisa disso.
