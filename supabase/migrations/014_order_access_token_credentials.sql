-- Replace booster-visible account credentials with an encrypted access token.
-- Customers still submit login/password after payment, but boosters only see a
-- bearer token. The token is the encrypted payload; plaintext credentials are
-- never stored in columns and are only resolvable by service_role for the game
-- launcher backend.
...
set search_path = public as $$
  select
    (p_service_id = 'elo_boost' and coalesce(p_boost_mode, 'solo') = 'solo')
    or p_service_id in ('win_boost', 'placement_matches', 'md5')
$$;
...
declare
  ...
  v_cipher bytea;
begin
  select id, customer_id, status, service_id, boost_mode
  into   v_order
  from   public.orders
  where  id = p_order_id
  for update;
  ...
  if not public.order_requires_access_token(v_order.service_id, v_order.boost_mode) then
...
-- Old rows used game_credentials as encrypted "login|password" and would not
-- be valid launcher tokens. Force regeneration in the new encrypted JSON token
-- format for eligible active orders.
update public.orders
...
where credentials_set = true
  ...
  and public.order_requires_access_token(service_id, boost_mode)
...
declare
  v_order record;
begin
  select id, assigned_booster_id, status, service_id, boost_mode, game_credentials, credentials_set
  into   v_order
  from   public.orders
  where  id = p_order_id;
...
-- Intended for the external launcher backend only. Do not call this from any
-- browser/client. It returns plaintext credentials after validating a token and
-- must remain service_role-only.
...
-- Paid eligible orders only enter the booster pool after the customer has
-- submitted credentials and the encrypted token exists. Duo boost and coaching
-- do not require customer account access and remain unaffected.
...
where status = 'awaiting_assignment'
  ...
  and (
    not public.order_requires_access_token(service_id, boost_mode)
...
-- Keep direct RPC accepts consistent with the view filter.
...
begin
  ...
  if public.order_requires_access_token(v_order.service_id, v_order.boost_mode)
