-- Achado MÉDIO da auditoria de pagamentos: quando process_mp_payment_event
-- (047, redefinida em 095) recusa um evento por 'payment_reconciliation_failed'
-- (valor/moeda do pagamento não bate com o total_price do pedido), a função
-- só retorna o erro pro webhook, que devolve 409 -- sem nenhum registro
-- persistido. O Mercado Pago fica reentregando a notificação indefinidamente
-- e o pedido trava em 'awaiting_payment' pra sempre, sem nenhum sinal visível
-- pro admin de que existe um problema. Isto adiciona uma notificação
-- broadcast pra todos os admins (mesmo padrão de request_order_support,
-- 083) na primeira vez que o mismatch é detectado -- guardado por
-- `not exists` pra não reenviar a cada re-entrega do webhook do mesmo evento.
