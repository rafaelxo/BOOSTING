-- Achado do checkup: em pedidos Win Boost/MD5, apply_order_drop (migration
-- 132) reduz total_price proporcionalmente ao progresso entregue
-- (v_new_total_price = total_price * (1 - completion_frac)) e zera
-- wins_played/losses_played ao reabrir o pedido, mas nunca reduz
-- wins_purchased -- o próximo booster continua obrigado a entregar a
-- quantidade ORIGINAL de vitórias, mesmo o preço já refletindo só a
-- fração restante. Isso permite o cliente receber mais vitórias do que
-- pagou, ou obriga o segundo booster a um volume de trabalho maior do que
-- a remuneração cobre.
--
-- Fix: reduzir wins_purchased pela mesma fração (1 - completion_frac)
-- aplicada a total_price/estimated_hours -- como completion_frac para
-- win_boost/md5 é exatamente wins_played/wins_purchased (order_drop_
-- completion_pct, migration 116), a nova meta é
-- wins_purchased - wins_played, o número de vitórias que de fato faltam.
-- Não afeta elo_boost (current_rank já é reancorado a cada drop) nem
-- clash/coaching/placement_matches (sem meta de vitórias).
